...
router.post("/channels", async (req, res, next) => {
  try {
    //get the data
    const { channelName, genre } = req.body;

    //save the data
    const newChannel = await Channel.create({ channelName, genre });
...
router.put("/channels/:channelId", async (req, res, next) => {
  try {
    ...
    const { channelName, genre, platform, allowedCountries } = req.body;

    const updatedChannel = await Channel.findByIdAndUpdate(
      channelId,
      { channelName, genre, platform, allowedCountries },
      { new: true }
    );
